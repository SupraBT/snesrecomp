...
/* True SPC:master ratio (SPC700 @ 1.024 MHz / SNES master @ 21.47727 MHz),
 * identical to RTL_APU_RATIO_NUM/DEN in common_rtl.c so the relative
 * catch-up and the absolute guest-clock anchor agree. The former
 * (32040*32)/(1364*262*60) = 17088/357368 was ~0.29% fast: the catch-up
 * overshot the anchor on long uploads, shifting the port-queue mapping and
 * the SPC's response phase (battle-streaming oscillation). */
static const double kInterpApuPerMaster = 5632.0 / 118125.0;
