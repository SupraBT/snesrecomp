...
/* Debug logging gate: set SNESRECOMP_PPU_DEBUG=1 to enable the register /
 * BG-state trace. Off by default — the trace fires for nearly every PPU
 * write and slows real runs down. */
static int ppu_debug_enabled(void) {
    static int v = -1;
    if (v < 0) v = getenv("SNESRECOMP_PPU_DEBUG") ? 1 : 0;
    return v;
}
...
/* PPU render cost split (cosim build only). Host-time (QPC, ~ns) accumulated
 * per phase so the field's ~23.7ms/frame SoDrawPpuFrame can be attributed to
 * sprite-eval / backgrounds / sprites / compose / HDMA. Inert everywhere else
 * (SNESRECOMP_INTERP_PROFILE is defined only in the cosim flags). */
static double g_ppu_sec_eval_ms = 0;   /* ppu_evaluateSprites              */
static double g_ppu_sec_line_ms = 0;   /* PpuDrawWholeLine whole            */
static double g_ppu_sec_bg_ms = 0;     /* PpuDrawBackgrounds (main+sub)     */
static double g_ppu_sec_spr_ms = 0;    /* PpuDrawSprites only               */
static double g_ppu_sec_compose_ms = 0;/* PpuDrawWholeLine minus bg (=compose/colormath/window) */
double g_ppu_sec_hdma_ms = 0;          /* SimpleHdma_DoLine loops (so_rtl)   */
...
void ppu_sec_reset(void) {
    g_ppu_sec_eval_ms = 0; g_ppu_sec_line_ms = 0;
    g_ppu_sec_bg_ms = 0; g_ppu_sec_spr_ms = 0;
    g_ppu_sec_compose_ms = 0; g_ppu_sec_hdma_ms = 0;
    g_ppu_sec_eval_n = 0; g_ppu_sec_line_n = 0;
}
void ppu_sec_read(double *eval, double *line, double *bg, double *spr,
                  double *compose, double *hdma) {
    *eval = g_ppu_sec_eval_ms; *line = g_ppu_sec_line_ms;
    *bg = g_ppu_sec_bg_ms; *spr = g_ppu_sec_spr_ms;
    /* whole line minus (backgrounds + sprites) = compositor/colormath/window. */
    *compose = g_ppu_sec_line_ms - g_ppu_sec_bg_ms - g_ppu_sec_spr_ms;
    if (*compose < 0.0) *compose = 0.0;
    *hdma = g_ppu_sec_hdma_ms;
}
/* PPU_T0/PPU_ACC: QPC timer pair accumulating into a named accumulator.
 * Each function that uses PPU_T0 declares `uint64_t _ppu_t;` at its top
 * (MSVC C89: no mid-block declarations); PPU_T0 is an expression statement. */
...
static int s_ppu_state_log_count = 0;
static void ppu_log_bg_state(Ppu *ppu) {
  if (!ppu_debug_enabled() || s_ppu_state_log_count >= 30) return;
  s_ppu_state_log_count++;
  fprintf(stderr, "[PPU_STATE] mode=%d bgTileAdr=$%04X bgSC=[%02X %02X %02X %02X]\n",
          PPU_mode(ppu), ppu->bgTileAdr,
          ppu->bgXsc[0], ppu->bgXsc[1], ppu->bgXsc[2], ppu->bgXsc[3]);
  fprintf(stderr, "[PPU_STATE] BG1: tile @$%04X map @$%04X | BG2: tile @$%04X map @$%04X\n",
          PPU_bgTileAdr(ppu, 0), PPU_bgTilemapAdr(ppu, 0),
          PPU_bgTileAdr(ppu, 1), PPU_bgTilemapAdr(ppu, 1));
  fprintf(stderr, "[PPU_STATE] BG3: tile @$%04X map @$%04X | BG4: tile @$%04X map @$%04X\n",
          PPU_bgTileAdr(ppu, 2), PPU_bgTilemapAdr(ppu, 2),
          PPU_bgTileAdr(ppu, 3), PPU_bgTilemapAdr(ppu, 3));
  fprintf(stderr, "[PPU_STATE] scroll: h0=%d v0=%d h1=%d v1=%d h2=%d v2=%d h3=%d v3=%d\n",
          ppu->hScroll[0], ppu->vScroll[0], ppu->hScroll[1], ppu->vScroll[1],
          ppu->hScroll[2], ppu->vScroll[2], ppu->hScroll[3], ppu->vScroll[3]);
}
...
void ppu_runLine(Ppu* ppu, int line) {
  ...
  if(line == 0) {
    ...
    // CGRAM palette protection: re-apply saved palette at pre-render line.
    // The game's DMA repeatedly writes zeros from empty WRAM $7E:D913.
    // By restoring at line 0, the palette is fresh when rendering starts.
    // DISABLED: causes infinite loop when game's DMA is blocked
    // if (sdd1_cgram_protected > 0) {
    //   memcpy(ppu->cgram, sdd1_cgram_save, sizeof(sdd1_cgram_save));
    //   sdd1_cgram_protected--;
    // }

    /* Decrement VRAM protection counter. Write guards in $2118/$2119
     * intercept zeroing writes to the tilemap area during protection. */
    // DISABLED: causes infinite loop when game's DMA is blocked
    // if (sdd1_vram_protected > 0) sdd1_vram_protected--;
    ...
    /* VRAM dump: log BG state in Mode 0, but ONLY after having been in Mode 1+ */
    if (ppu_debug_enabled()) {
      static int ever_left_m0 = 0;
      if (PPU_mode(ppu) != 0) ever_left_m0 = 1;
      if (line == 1 && PPU_mode(ppu) == 0 && ever_left_m0) {
        static int m0_dump_count = 0;
        if (m0_dump_count < 5) {
          m0_dump_count++;
          fprintf(stderr, "[VRAM_M0] Mode0 after Mode1+ (dump %d):\n", m0_dump_count);
          fprintf(stderr, "  bgmode=$%02X bgTileAdr=$%04X bgSC=[%02X %02X %02X %02X]\n",
                  ppu->bgmode, ppu->bgTileAdr,
                  ppu->bgXsc[0], ppu->bgXsc[1], ppu->bgXsc[2], ppu->bgXsc[3]);
          fprintf(stderr, "  WRAM[$0601]=$%02X (bit7=%d)\n",
                  g_snes->ram[0x0601], (g_snes->ram[0x0601] >> 7) & 1);
          fprintf(stderr, "  scroll: h0=%d v0=%d h1=%d v1=%d\n",
                  ppu->hScroll[0], ppu->vScroll[0], ppu->hScroll[1], ppu->vScroll[1]);
          for (int bg = 0; bg < 4; bg++) {
            int mapadr = PPU_bgTilemapAdr(ppu, bg);
            int tileadr = PPU_bgTileAdr(ppu, bg);
            fprintf(stderr, "  BG%d: map @$%04X tiles @$%04X -> ", bg, mapadr, tileadr);
            for (int i = 0; i < 8; i++)
              fprintf(stderr, "%04X", ppu->vram[(mapadr + i) & 0x7fff]);
            fprintf(stderr, "\n");
          }
          fprintf(stderr, "  VERIFY: VRAM[$4800]=$%04X VRAM[$4C00]=$%04X\n",
                  ppu->vram[0x4800], ppu->vram[0x4C00]);
        }
      }
    }
    // evaluate sprites
    ...
    /* PPU cost split (cosim build): sprite-eval vs whole-line vs the bg sub-split
     * accumulated inside PpuDrawWholeLine; compose = line_ms - bg_ms. */
    PPU_T0; ppu->lineHasSprites =
        !PPU_forcedBlank(ppu) && ppu_evaluateSprites(ppu, line - 1);
    PPU_ACC(g_ppu_sec_eval_ms);
#ifdef SNESRECOMP_INTERP_PROFILE
    g_ppu_sec_eval_n++;
#endif
...
static void PpuDrawBackgrounds(Ppu *ppu, int y, bool sub) {
  ...
  if (PPU_mode(ppu) == 0) {
    // Mode 0: ALL 4 BGs are 2bpp (4 colors each, 16 bytes/tile)
    // Priority order (SNES hardware):
    //  Prio1: OBJ-3 > BG1-1 > BG2-1 > BG3-1 > BG4-1
    //  Prio0: OBJ-2 > BG1-0 > BG2-0 > OBJ-1 > BG3-0 > BG4-0 > OBJ-0
    if (ppu->lineHasSprites) { PPU_T0; PpuDrawSprites(ppu, y, sub, true); PPU_ACC(g_ppu_sec_spr_ms); }

    bool mosaic_size = PPU_mosaicSize(ppu) > 1;
    // BG1 (layer 0): 2bpp, zhi=prio1 (0xC000), zlo=prio0 (0x8000)
    ...
    // BG2 (layer 1): 2bpp, zhi=prio1 (0xB100), zlo=prio0 (0x7100)
    ...
    // BG3 (layer 2): 2bpp, zhi=prio1 (0x3200), zlo=prio0 (0x2200)
    layerbuf = PpuBeginBackgroundOverlay(ppu, y, sub, 2);
    PpuDrawBackground_2bpp_policy(
        ppu, layerbuf, y, sub, 2, 0x3200, 0x2200,
        mosaic_size && PPU_mosaicEnabled(ppu, 2));
    PpuFinishBackgroundOverlay(ppu, y, sub, 2, layerbuf);

    // BG4 (layer 3): 2bpp, zhi=prio1 (0x1300), zlo=prio0 (0x0900).
    // zlo must stay above the backdrop marker (0x0500) or BG4-0 pixels
    // lose the z compare and are never drawn (Star Ocean name-screen
    // lettering is BG4-0 over the teal backdrop cgram[0]).
    layerbuf = PpuBeginBackgroundOverlay(ppu, y, sub, 3);
    PpuDrawBackground_2bpp_policy(
        ppu, layerbuf, y, sub, 3, 0x1300, 0x0900,
        mosaic_size && PPU_mosaicEnabled(ppu, 3));
    PpuFinishBackgroundOverlay(ppu, y, sub, 3, layerbuf);
    ...
    PpuFinishBackgroundOverlay(ppu, y, sub, 0, layerbuf);
    if (ppu->lineHasSprites) { PPU_T0; PpuDrawSprites(ppu, y, sub, false); PPU_ACC(g_ppu_sec_spr_ms); }
  }
}
...
static NOINLINE void PpuDrawWholeLine(Ppu *ppu, uint y) {
  ...
  // Render main screen
  PPU_T0; PpuDrawBackgrounds(ppu, y, false); PPU_ACC(g_ppu_sec_bg_ms);
  if (ppu->widescreenLineEnhancer &&
      (ppu->extraLeftCur || ppu->extraRightCur))
  ...
  if (PPU_preventMathMode(ppu) != 3 && PPU_addSubscreen(ppu) && PPU_mathEnabled(ppu)) {
    ...
    if (ppu->screenEnabled[1] != 0) {
      PPU_T0; PpuDrawBackgrounds(ppu, y, true); PPU_ACC(g_ppu_sec_bg_ms);
      if (ppu->widescreenLineEnhancer &&
          (ppu->extraLeftCur || ppu->extraRightCur))
...
//    printf("ppu_write(%d, %d)\n", adr, val);
  switch(adr) {
    case INIDISP & 0xff:
      ppu->inidisp = val;
      { /* env-gated $2100 trace (SNESRECOMP_INIDISP_TRACE=1) */
        static int tr = -1;
        if (tr < 0) { const char *e = getenv("SNESRECOMP_INIDISP_TRACE");
                      tr = (e && e[0] && e[0] != '0') ? 1 : 0; }
        if (tr) {
          extern int snes_frame_counter;
          extern uint32_t g_interp816_cur_pc;
          fprintf(stderr, "[inidisp] f=%d pc=%06X val=%02X\n",
                  snes_frame_counter, (unsigned)g_interp816_cur_pc, val);
        }
      }
      break;
    case OBSEL & 0xff:
    ...
    case BGMODE & 0xff: {
      assert((val & 0xf0) == 0);
      uint8_t oldmode = ppu->bgmode;
      ppu->bgmode = val;
      if (val != oldmode) {
        if (ppu_debug_enabled())
          fprintf(stderr, "[PPU] BGMODE %d->%d (raw $%02X->$%02X) WRAM[0601]=$%02X\n", oldmode & 7, val & 7, oldmode, val, g_snes->ram[0x0601]);
        /* FIX: When entering Mode 0, set WRAM[$0601] bit 7.
         * On real SNES the game's VBLANK handler ($CA:4B26: lda #$80; sta $0601)
         * sets this flag before the TRB $0601 check at $CA:6434 gates S-DD1.
         * In our recompiler the game loop never reaches $CA:4B26, so we emulate
         * the side-effect here to unblock the Mode 0 S-DD1 tilemap decompression. */
        if (val == 0 && oldmode != 0)
          g_snes->ram[0x0601] |= 0x80;
      }
      break;
    }
    case MOSAIC & 0xff:
    ...
    case BG4SC & 0xff: {
      int bg = adr - 7;
      if (ppu_debug_enabled())
        fprintf(stderr, "[PPU] BG%dSC=$%02X (tilemap @$%04X)\n", bg, val, (val & 0xfc) << 8);
      ppu->bgXsc[bg] = val;
      /* SDD1_MODE0 hard-coded tilemap/palette decompression removed: the
       * game's own DMA->S-DD1 path serves these tilemaps through the MMC
       * window (engine validated byte-exact vs bsnes; see sdd1_compare.py). */
      break;
    }
    case BG12NBA & 0xff:
      ppu->bgTileAdr = ppu->bgTileAdr & 0xff00 | val;
      if (ppu_debug_enabled())
        fprintf(stderr, "[PPU] BG12NBA=$%02X -> BG1 tiles @$%04X BG2 tiles @$%04X\n",
                val, (val & 0x0f) << 12, (val >> 4) << 12);
      break;
    case BG34NBA & 0xff:
      ppu->bgTileAdr = ppu->bgTileAdr & 0xff | val << 8;
      if (ppu_debug_enabled())
        fprintf(stderr, "[PPU] BG34NBA=$%02X -> BG3 tiles @$%04X BG4 tiles @$%04X\n",
                val, (val & 0x0f) << 12, (val >> 4) << 12);
      break;
    case 0x0d: {
    ...
    case 0x16: {
      ...
      ppu->vramReadBuffer = ppu->vram[ppu_getVramRemap(ppu) & 0x7fff];
      if (ppu_debug_enabled()) {
        static int vmadd_log = 0;
        if (vmadd_log < 10) {
          fprintf(stderr, "[VMADD] $2116=$%02X -> ptr=$%04X\n", val, ppu->vramPointer);
          vmadd_log++;
        }
      }
      break;
    ...
    case 0x17: {
      ...
      ppu->vramReadBuffer = ppu->vram[ppu_getVramRemap(ppu) & 0x7fff];
      if (ppu_debug_enabled()) {
        static int vmadd_log2 = 0;
        if (vmadd_log2 < 10) {
          fprintf(stderr, "[VMADD] $2117=$%02X -> ptr=$%04X\n", val, ppu->vramPointer);
          vmadd_log2++;
        }
      }
      break;
    ...
    case 0x15: {
      ...
      if (ppu_debug_enabled()) {
        static int vmain_log = 0;
        if (vmain_log < 20) {
          fprintf(stderr, "[VMAIN] $2115=$%02X inc=%d remap=%d incOnHigh=%d\n",
                  val, ppu->vramIncrement, ppu->vramRemapMode, ppu->vramIncrementOnHigh);
          vmain_log++;
        }
      }
      break;
    ...
    case 0x18: {
      ...
      /* VRAM write protection: block zeroing writes to tilemap range */
      // DISABLED: causes infinite loop
      // if (sdd1_vram_protected > 0 && val == 0 &&
      //     (vramAdr & 0x7fff) >= sdd1_vram_lo && (vramAdr & 0x7fff) <= sdd1_vram_hi) {
      ...
      WsShadowOnVramWrite((uint16_t)(vramAdr & 0x7fff), ppu->vram[vramAdr & 0x7fff]);
      if (ppu_debug_enabled()) {
        static int vram_w18 = 0;
        if (val != 0 && vram_w18 < 40) {
          fprintf(stderr, "[VRAM_W] $2118=$%02X @VRAM[$%04X]\n", val, vramAdr);
          vram_w18++;
        }
      }
      if(!ppu->vramIncrementOnHigh) ppu->vramPointer += ppu->vramIncrement;
    ...
    case 0x19: {
      ...
      WsShadowOnVramWrite((uint16_t)(vramAdr & 0x7fff), ppu->vram[vramAdr & 0x7fff]);
      if (ppu_debug_enabled()) {
        static int vram_w19 = 0;
        if (val != 0 && vram_w19 < 40) {
          fprintf(stderr, "[VRAM_W] $2119=$%02X @VRAM[$%04X]\n", val, vramAdr);
          vram_w19++;
        }
      }
      if(ppu->vramIncrementOnHigh) ppu->vramPointer += ppu->vramIncrement;
    ...
    case 0x21:
      ...
      if (ppu_debug_enabled()) {
        static int cgram_addr_log = 0;
        if (cgram_addr_log < 20) {
          fprintf(stderr, "[CGRAM_ADDR] $2121=$%02X\n", val);
          cgram_addr_log++;
        }
      }
      break;
    case 0x22:
      if(!ppu->cgramSecondWrite) {
        ...
        /* CGRAM write protection: after SDD1_MODE0 loads the name screen
         * palette, the game's DMA writes zeros from empty WRAM $7E:D913.
         * Block ALL zeroing writes during protection. The saved palette
         * is re-applied at line 0 of each frame. */
        // DISABLED: causes infinite loop
        // extern int sdd1_cgram_protected;
        // if (sdd1_cgram_protected > 0) {
        //   uint16_t cgramWord = (val << 8) | ppu->cgramBuffer;
        //   if (cgramWord == 0) {
        //     ppu->cgramPointer++;
        //     ppu->cgramSecondWrite = !ppu->cgramSecondWrite;
        //     break;
        ...
        // }
        uint16_t cgramWord = (val << 8) | ppu->cgramBuffer;
        uint8_t idx = ppu->cgramPointer;
        ppu->cgram[idx] = cgramWord;
        ppu->cgramPointer++;
        /* Log backdrop (color 0) and first few sprite palette writes */
        if (ppu_debug_enabled()) {
          static int cgram_log = 0;
          if (cgram_log < 40 && (idx < 32 || (idx >= 128 && idx < 140))) {
            fprintf(stderr, "[CGRAM_W] [%d] = $%04X (R=%d G=%d B=%d)\n",
                    idx, cgramWord,
                    (cgramWord >> 0) & 0x1f,
                    (cgramWord >> 5) & 0x1f,
                    (cgramWord >> 10) & 0x1f);
